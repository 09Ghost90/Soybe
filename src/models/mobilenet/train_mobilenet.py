...
# Classificação Supervisionada de Imagens de Soja usando MobileNetV3-Large

# TensorBoard opcional
writer = SummaryWriter() if SummaryWriter is not None else None

# Hiperparâmetros
batch_size = 32  # MobileNet é leve, suporta batches maiores
num_epochs = 20
...
# MobileNetV3 usa entrada 224x224 — arquitetura otimizada para dispositivos leves
if device.type == "cpu":
    batch_size = 8
    print("CPU mode: batch_size ajustado para 8 (MobileNet é leve).")
...
# Modelo MobileNetV3-Large pré-treinado
weights = MobileNet_V3_Large_Weights.IMAGENET1K_V2
model = torchvision.models.mobilenet_v3_large(weights=weights)

# MobileNetV3 usa model.classifier[-1] como última camada (similar ao EfficientNet)
...
if __name__ == "__main__":
    ...
    print(f"Tempo total de treinamento: {end - start:.2f}s")
    ...
    # Avaliação do Modelo
    acc = Accuracy(task="multiclass", num_classes=num_class, average="macro").to(device)
